...
const RestaurantPage = async ({ params }: RestaurantPageProps) => {
  ...
  const serializedRestaurant = serializePrismaObject(restaurant);
      ...
      <div className="mt-0 md:mt-7">
        <div className="flex flex-col gap-0 md:flex-row md:gap-6 md:px-5">
          ...
          <div className=" rounded-lg border border-solid shadow-sm md:w-1/2 md:py-6">
            <div className="relative z-50 mt-[-1.5rem] flex items-center justify-between rounded-tl-3xl rounded-tr-3xl bg-background px-5 pt-5">
              <div>
            ...
            <div className="px-5">
              <DeliveryInfo restaurant={serializedRestaurant} />
            </div>
            ...
            <div className="mt-3 flex gap-4 overflow-x-scroll px-5 [&::-webkit-scrollbar]:hidden">
              {restaurant.categories.map((category) => (
                <div
                  key={category.id}
                  className="min-w-[167px] rounded-lg bg-muted text-center"
                >
        ...
        <CartBanner restaurant={serializedRestaurant} />
      </div>
    </>
  );
};
